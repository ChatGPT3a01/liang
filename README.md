...
**簡單任務用 Codex（省錢）｜困難任務用 Claude（品質）｜全自動模式一鍵放飛 ⚡**
...
全自動模式（跳過所有確認，適合信任的批次任務）：

liang › /cld 重構所有測試檔案          ← ⚡ Claude 全自動（無權限詢問）
liang › /cod fix all lint errors       ← ⚡ Codex 全自動（無沙盒限制）
```
...
| ⚡ `/cld` | Claude Code **全自動模式**（跳過權限確認＋無閃爍） | `/cld` |
| ⚡ `/cld <提示詞>` | 帶提示詞啟動 Claude 全自動 | `/cld 重構所有測試` |
| 🟢 `/co` | 啟動 Codex CLI（互動模式） | `/co` |
| 🟢 `/co <提示詞>` | 帶提示詞啟動 Codex CLI | `/co fix the login bug` |
| ⚡ `/cod` | Codex CLI **全自動模式**（跳過審核與沙盒） | `/cod` |
| ⚡ `/cod <提示詞>` | 帶提示詞啟動 Codex 全自動 | `/cod 修好所有 lint` |
